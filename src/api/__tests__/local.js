...
describe('Local API Wrapper', () => {
  it('Gets a person', async () => {
    var luke =
      JSON.parse(await getFromLocalUrl('http://swapi.co/api/people/1/'));
    expect(luke.name).to.equal('Luke Skywalker');
    var threePO =
      JSON.parse(await getFromLocalUrl('http://swapi.co/api/people/2/'));
    expect(threePO.name).to.equal('C-3PO');
  });

  it('Gets pages', async () => {
    var firstPeople =
      JSON.parse(await getFromLocalUrl('http://swapi.co/api/people/?page=1'));
    expect(firstPeople.results.length).to.equal(10);
    expect(firstPeople.results[0].name).to.equal('Luke Skywalker');
    var secondPeople =
      JSON.parse(await getFromLocalUrl('http://swapi.co/api/people/?page=2'));
  ...
  it('Gets first page by default', async () => {
    var people =
      JSON.parse(await getFromLocalUrl('http://swapi.co/api/people/'));
